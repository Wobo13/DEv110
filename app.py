...
# --- 7. START (V2.8 - Content-Rich Dashboard: Trivia & Spotlight) ---
...
if current_choice == "🏠 Start":
    ...
    # Filtrowanie słówek do powtórki i do warsztatu (level 0-1)
    ...
    col2.metric("Do powtórki", len(due_cards), delta=-len(due_cards) if due_cards else "Czysto!", delta_color="inverse")
    ...
    with c1:
        # --- A. SŁÓWKO DNIA (SPOTLIGHT) ---
        st.markdown(f"### 🎯 Słówko pod lupą")
        if hard_cards:
            # Stały wybór na 24h z Twojego Warsztatu
            idx_spot = int(hashlib.md5((today_str + "spot").encode()).hexdigest(), 16) % len(hard_cards)
            spot_word = hard_cards[idx_spot]
            
            with st.container(border=True):
                st.markdown(f"<h2 style='color:#4CAF50; margin-top:0; margin-bottom:0;'>{spot_word['de']}</h2>", unsafe_allow_html=True)
                st.caption("To słowo sprawia Ci trudność. Opanuj je dzisiaj!")
                
                with st.expander("👁️ Sprawdź znaczenie"):
                    st.write(f"🎯 **{spot_word['pl']}**")
                
                st.write("**🗣️ Shadowing:**")
                st.caption("Odsłuchaj i powtórz 5 razy na głos, naśladując akcent.")
                if st.button("🔊 Odsłuchaj wymowę", key="spot_audio", use_container_width=True):
                    play_audio(spot_word['de'], lang=L_CODE)
        else:
            st.success("✨ Twoja lista trudnych słówek jest pusta! Dobra robota.")

        # --- B. KULTURA W PIGUŁCE ---
        st.markdown(f"### 🥨 Kultura w pigułce")
        try:
            db = get_db()
            ...
            if res_trivia.data:
                ...
                with st.container(border=True):
                    st.markdown(f"**{trivia['title']}**")
                    st.write(trivia['content_orig'])
                    with st.expander("👁️ Tłumaczenie"):
                        st.caption(trivia['content_pl'])
            else:
                st.info("Baza ciekawostek jest pusta. Użyj panelu Admina, aby ją zainicjować.")
        except:
            st.error("Błąd połączenia z bazą ciekawostek.")
    ...
    with c2:
        ...
        try:
            ...
            # LOGIKA WARSZTATU (Language Specific)
            wrk_goal = 3
            wrk_key = f"wrk_mastered_today_{L_CODE}"
            mastered_today = st.session_state.get(wrk_key, 0)
            workshop_done = mastered_today >= wrk_goal
            hard_cards_count = len(hard_cards)
        ...
        with st.container(border=True):
            st.markdown(f"**{'✅' if writing_done else '✍️'} Asystent Pisania**")
            ...
            else: st.caption("✨ Zadanie zaliczone!")

        with st.container(border=True):
            st.markdown(f"**{'✅' if det_done else '🕵️'} Kulturowy Detektyw**")
            ...
            else: st.caption("✨ Sprawa rozwiązana!")

        with st.container(border=True):
            st.markdown(f"**{'✅' if workshop_done else '🛠️'} Warsztat Słówek**")
            if hard_cards_count > 0:
                st.markdown(f"📊 *Postęp: {mastered_today}/{wrk_goal} słów*")
                if not workshop_done:
                    if st.button("Otwórz Warsztat", key="go_to_wrk", use_container_width=True):
                        st.session_state.choice = "🛠️ Warsztat"; st.rerun()
                else: st.caption("✨ Warsztat odpracowany!")
            else:
                st.markdown("✨ *Wszystkie słowa są opanowane!*")

    st.divider()

    # 4. CYTAT DNIA I OSTATNIE SŁÓWKA
    col_q, col_w = st.columns([2, 1])
    with col_q:
        quotes_db = {
            "de": [{"orig": "Die Grenzen meiner Sprache bedeuten die Grenzen meiner Welt.", "pl": "Granice mojego języka oznaczają granice mojego świata."}, {"orig": "Übung macht den Meister!", "pl": "Praktyka czyni mistrza."}],
...
# --- 21. GENERATOR SŁÓW (V253 - Clean Polish & Forced CEFR Levels) ---
elif choice == "📦 Generator":
    current_lang_name = st.session_state.get("current_lang", "Niemiecki")
    L_CODE = "de" if current_lang_name == "Niemiecki" else "cs"
    
    st.header(f"📦 Generator: {current_lang_name}")
    st.write(f"Generuj słówka. System zadba o czyste tłumaczenia i poziomy trudności.")

    # 1. PANEL STEROWANIA
    with st.container(border=True):
        c1, c2, c3 = st.columns([2, 2, 1])
        with c1:
            gen_lvl = st.selectbox("Preferowany poziom:", ["Brak", "A1", "A2", "B1", "B2", "C1", "C2"], key="gen_lvl_sel")
        with c2:
            gen_topic = st.text_input("Temat (opcjonalnie):", placeholder="np. Dom, Technika...", key="gen_top_in")
        with c3:
        ...
        if st.button(f"✨ Generuj listę ({current_lang_name})", use_container_width=True, type="primary"):
            with st.spinner(f"AI dobiera słownictwo ({current_lang_name})..."):
                ...
                # --- PRECYZYJNE INSTRUKCJE DLA AI ---
                if L_CODE == "de":
                    lang_instr = """
                    1. RZECZOWNIKI NIEMIECKIE: Zawsze z rodzajnikiem określonym (der, die, das).
                    2. TŁUMACZENIE POLSKIE: Podaj czysty polski odpowiednik (BEZ słów ten, ta, to).
                    """
                else: # cs
                    lang_instr = """
                    1. RZECZOWNIKI CZESKIE: Zawsze z zaimkiem wskazującym (ten, ta, to).
                    2. TŁUMACZENIE POLSKIE: Podaj czysty polski odpowiednik (BEZ słów ten, ta, to). To bardzo ważne!
                    """

                prompt = f"""Wygeneruj {gen_count} unikalnych słówek/fraz w języku {current_lang_name} {context}.
                {lang_instr}
                
                WYMÓG KATEGORII: Do pola 'tags' (kategorie) OBOWIĄZKOWO dodaj poziom trudności (A1, A2, B1, B2, C1 lub C2) pasujący do słowa.
                
                Dla każdego elementu podaj:
                1. de: słowo (oryginał z rodzajnikiem/zaimkiem jeśli to rzeczownik)
                2. pl: tłumaczenie na polski (czyste, bez rodzajników)
                3. tags: lista min. 3 kategorii (w tym obowiązkowo poziom CEFR)
                4. ex_de: przykład użycia w oryginale
    ...
    if "temp_generated" in st.session_state and st.session_state.temp_generated:
        ...
        for item in st.session_state.temp_generated:
            # Upewniamy się, że poziom z selectboxa jest w tagach, jeśli AI by zapomniało
            tags = item.get("tags", "")
            current_sel_lvl = st.session_state.get("last_gen_lvl", "Brak")
            if current_sel_lvl != "Brak" and current_sel_lvl not in tags:
                tags = f"{current_sel_lvl}, {tags}"
        ...
        edited_df = st.data_editor(df_init, use_container_width=True, num_rows="dynamic", key="ai_editor_gen_v253")

        col_save, col_cancel = st.columns(2)
        if col_save.button(f"🚀 Zapisz do bazy ({current_lang_name})", use_container_width=True, type="primary"):
            success_count = 0
            for row in edited_df:
                if row.get("Dodaj", False):
                    new_word = {
                        "de": row[lang_col], "pl": row["Polski"], "category": row["Kategorie"],
                        "next_review": str(date.today()), "level": 0, "origin": "Generator",
                        "lang": L_CODE, "examples": [{"de": row["Przykład (Oryg.)"], "pl": row["Przykład (PL)"]}]
                    }
                    save_word(u, new_word)
                    success_count += 1
            st.success(f"Dodano {success_count} słówek!"); st.session_state.flashcards = load_flashcards(u)
            if "temp_generated" in st.session_state: del st.session_state.temp_generated
            st.rerun()
