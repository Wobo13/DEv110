...
# --- 27. ADMIN PRO (V550 - User Control & Profile Editor) --- 
 ...
             prompt = f"Wygeneruj {batch_size} unikalnych słów 
{target_lvl} ({target_lang}). JSON: vocab: [{{word, translation, level, 
lang, category, example_orig, example_pl}}]" 
             try: 
                 raw_res = get_openai_response(prompt) 
...
pd.DataFrame(columns=["username", "mastery_xp", "origin"]) 
 ...
                     admin_notes = st.text_area("Notatki Admina 
(niewidoczne dla użytkownika):", value=target_data.get("admin_notes", 
"")) 
                  
                 with col_ed2: 
                     is_banned = st.checkbox("🚫 Zablokuj konto (BAN)", value=target_data.get("is_banned", False)) 

                     is_shadow = st.checkbox("👻 Shadowban (ukryj z 
rankingów)", value=target_data.get("is_shadowbanned", False)) 
