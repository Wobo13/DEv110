...
def robust_parse_json(raw_text):
    try:
        ...
        code_block = re.search(r'\[.*\]', raw_text, re.DOTALL)
        if code_block:
            clean_json = code_block.group(0).replace('```json', '').replace('```', '').strip()
            return json.loads(clean_json)
        # 2. Jeśli nie ma bloku, próbujemy załadować surowy tekst
        return json.loads(raw_text.strip())
    except Exception as e:
        st.error(f"Błąd parsowania: {e}")
        with st.expander("Zobacz co odpowiedziało AI (Debug)"):
            st.code(raw_text)
        return None
...
if not st.session_state.auth:
    ...
    with t2:
        unew = st.text_input("Nowy użytkownik", key="r_u").lower().strip()
        pnew = st.text_input("Hasło", type="password", key="r_p")
        if st.button("Załóż konto", use_container_width=True):
            db = load_j(AUTH_FILE, {})
            if unew and len(pnew) >= 4 and unew not in db:
                db[unew] = hash_pw(pnew); save_j(AUTH_FILE, db)
                save_j(get_p(unew, "flashcards"), [])
                save_j(get_p(unew, "user_data"), {"streak":0, "historical_cost": 0.0, "time_stats": {}, "last_ts": time.time(), "last_seen": "Nigdy"})
...
if choice in ["📅 Powtórki", "🚀 Trening"]:
    ...
    if not cards: st.success("Czysto! 🎉")
        ...
        if st.session_state.n_m == "ask":
            with st.form("ans_f"):
                uai = st.text_input("Twoja odpowiedź:")
                if st.form_submit_button("Sprawdź", use_container_width=True):
                    st.session_state.u_a = uai; st.session_state.n_m = "res"; st.rerun()
        else:
            ans = st.session_state.get("u_a", "")
            is_ok = ans.strip().lower() in [s.strip().lower() for s in re.split(r'[/,;]', c['pl'])]
            if is_ok: st.success(f"✅ Dobrze: {c['pl']}")
    ...
    if cards:
        ...
        if st.session_state.f_flipped and c.get("examples"):
            for ex in c["examples"]: ex_html += f"<div style='margin-top:15px; border-top:1px solid #444; padding-top:10px;'><span style='color:#FFEB3B; font-weight:bold;'>🇩🇪 {ex['de']}</span><br><span style='color:white; font-style:italic;'>🇵🇱 {ex['pl']}</span></div>"
        st.markdown(f'<div style="min-height:350px; display:flex; flex-direction:column; align-items:center; justify-content:center; background:black; border:2px solid red; border-radius:30px; padding:30px; text-align:center;"><h1 style="color:white; margin:0; font-size:2.2em;">{word_txt}</h1>{ex_html}</div>', unsafe_allow_html=True)
        st.write("<br>", unsafe_allow_html=True); c1, c2, c3 = st.columns([1, 1.2, 1])
...
# --- MODUŁ: SKANER AI (Gemini 1.5 Flash + Robust Parser) ---
    ...
    if (src or up) and st.button("🚀 ANALIZUJ", use_container_width=True):
        try:
            genai.configure(api_key=API_KEY); m = genai.GenerativeModel('gemini-1.5-flash')
            res = m.generate_content(["Extract vocabulary. Return JSON list: [{'de':'...', 'pl':'...', 'category':'Skaner', 'examples':[{'de':'...', 'pl':'...'}]}]", Image.open(src or up).convert("RGB")])
            data = robust_parse_json(res.text)
            if data: st.session_state.pending = data; st.session_state.user_data["historical_cost"] += 0.005; st.rerun()
        except Exception as e: st.error(f"Błąd połączenia: {e}")
...
# --- MODUŁ: GENERATOR SŁÓW (Gemini 1.5 Flash + Robust Parser) ---
        ...
        if cols[i].button(lvl, use_container_width=True):
            with st.spinner("AI generuje..."):
                try:
                    ...
                    p = f"Generate 25 unique German words for level {lvl}. Categories/Examples in Polish. Skip: {exist}. Return ONLY JSON list: [{{'de':'...', 'pl':'...', 'category':'...', 'examples':[{{'de':'...', 'pl':'...'}}]}}]"
                    res = m.generate_content(p); data = robust_parse_json(res.text)
                    if data:
                        for w in data:
                            if w['de'].lower() not in [x['de'].lower() for x in st.session_state.flashcards]:
                                ...
                                st.session_state.flashcards.append(w)
                        st.session_state.user_data["historical_cost"] += 0.003; save_j(get_p(u, "flashcards"), st.session_state.flashcards); st.rerun()
...
# --- MODUŁ: ADMIN (PEŁNY) ---
    ...
    for usr in users:
        ...
        adm_data.append({"Użytkownik": usr, "Słów": len(ub), "AI (Skaner)": ai_n, "% Wiedzy": mastery, "Ostatnio": ud.get("last_seen", "Nigdy"), "Aktywność": u_times or "Brak", "Koszt": ud.get("historical_cost",0.0)})
    
    m1.metric("Łącznie Słówek", sum(x['Słów'] for x in adm_data))
    m2.metric("Bonus AI", f"{BONUS_START - sum(x['Koszt'] for x in adm_data):.2f} PLN")
    st.subheader("👥 Baza Użytkowników"); st.dataframe(pd.DataFrame(adm_data).drop(columns=['Koszt']), use_container_width=True)
    ...
    if total_g > 0:
        ...
        fig.update_layout(template="plotly_dark", height=400, margin=dict(l=10, r=10, t=30, b=10)); st.plotly_chart(fig, use_container_width=True)

# --- RESZTA MODUŁÓW (STABILNE) ---
elif choice == "🕹️ Quiz":
    ...
    with st.form("add"):
        de = st.text_input("Niemiecki"); pl = st.text_input("Polski"); kat = st.text_input("Kategoria")
        ...
        if (f_kat == "Wszystkie" or c.get("category") == f_kat) and (search.lower() in c['de'].lower()):
            with st.expander(f"📝 {c['de']} — {c['pl']}"):
                with st.form(f"ed_{i}"):
                    n_de, n_pl = st.text_input("DE", c['de']), st.text_input("PL", c['pl'])
                    if st.form_submit_button("Zapisz", use_container_width=True):
    ...
    st.divider(); st.subheader("⚠️ Strefa Niebezpieczna"); conf = st.checkbox("Potwierdzam chęć usunięcia danych")
    ...
    if st.button("🗑️ USUŃ WSZYSTKO (CZYŚĆ BAZĘ)", type="primary", disabled=not conf, use_container_width=True):
