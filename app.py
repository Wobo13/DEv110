...
# --- KONFIGURACJA APKI ---
APP_VERSION = "V199 (Supreme Restore - Supabase Engine)"
ADMIN_USER = "wobo"
API_KEY = st.secrets.get("OPENAI_API_KEY", "")

# MAPOWANIE NAZW MODUŁÓW (Ujednolicenie skrótów dla Admina)
CLEAN_TIME_LABELS = {
...
VOCAB_DB = {
    ...
    "B1": [
        ...
        "Abhängig", "Ablehnen", "Abmachen", "Abnehmen", "Abonnieren", "Absagen", "Abschließen", "Absolvieren", "Abstimmen", "Abwarten", "Achten", "Ändern", "Anerkennen", "Anfangen", "Anfordern", "Angeben", "Angehören", "Angreifen", "Anhalten", "Anklagen", "Ankommen", "Ankündigen", "Anmelden", "Annehmen", "Anpassen", "Anrufen", "Anschließen", "Ansehen", "Ansprechen", "Anstrengen", "Antworten", "Anwenden", "Anzeigen", "Anziehen", "Arbeiten", "Ärgern", "Aufbauen", "Aufgeben", "Aufhalten", "Aufhören", "Aufklären", "Aufmerksam", "Aufaufnahme", "Aufräumen", "Aufregen", "Aufstehen", "Aufutreten", "Aufwachen", "Ausbilden", "Ausdehnen",
...
def load_flashcards(username):
    ...
    # Inteligentna migracja pola origin (dla znajomych)
    for c in cards:
        if not c.get("origin"):
...
if choice in ["📅 Powtórki", "🚀 Trening"]:
    ...
    if len(all_c) < 4: st.warning("Dodaj min. 4 słówka!")
        ...
        if st.session_state.q_s == "ask":
            ...
            if is_correct(st.session_state.get("u_q"), st.session_state.q_a):
                ...
                if str(c.get("next_review", str(today_dt))) <= str(today_dt):
    ...
    update_activity("Fiszki"); all_cats = sorted(list(set([str(c.get("category", "Inne")) for c in st.session_state.flashcards])))
    sel_kat = st.selectbox("🎯 Kategoria:", ["Wszystkie"] + all_cats)
    cards = [c for c in st.session_state.flashcards if sel_kat == "Wszystkie" or str(c.get("category")) == sel_kat]
    if cards:
        ...
        if "test_q" not in st.session_state:
            ...
            if st.button("🚀 GENERUJ TEST", use_container_width=True, type="primary"):
                with st.spinner("AI przygotowuje zadania z polskim kontekstem..."):
                    ...
                    prompt = f"Generate EXACTLY {len(sample)} German questions for: {words_str}. Provide 'hint' (Polish context), 'sentence', 'correct' (DE). For QUIZ provide 3 'distractors'. JSON key 'questions'."
                    try:
            ...
            if t_idx < len(qs):
                ...
                total = len(qs)
                score = st.session_state.test_score; perc = round((score/total)*100) if total > 0 else 0
...
# --- NARZĘDZIA ---
    ...
    if "pending" in st.session_state:
        ...
        if st.button("✅ ZAPISZ DO BAZY CHMUROWEJ", use_container_width=True):
            for w in ed.to_dict('records'):
                if 'de' in w and 'pl' in w: save_word(u, {"de":w['de'], "pl":w['pl'], "category":w.get('category','Skaner'), "next_review":str(today_dt), "origin":"Skaner", "examples":w.get('examples', [])})
            del st.session_state.pending; st.success("Zapisano!"); time.sleep(1); st.rerun()

elif choice == "📦 Generator słów":
    update_activity("Generator"); lvls = ["A1", "A2", "B1", "B2", "C1"]; cols = st.columns(5)
    if "gen_msg" in st.session_state:
        st.success(st.session_state.gen_msg); del st.session_state.gen_msg
        ...
        if cols[i].button(lvl, use_container_width=True):
            with st.spinner(f"Pobieram 25 słówek {lvl}..."):
                try:
                    ...
                    selection = random.sample(available, min(25, len(available)))
                    prompt = f"Translate to Polish: {selection}. Provide Polish categories and 1 example sentence for each. JSON key 'flashcards'."
                    res = get_openai_response(prompt); data = json.loads(res)
                    if isinstance(data, dict) and "flashcards" in data:
                        for w in data["flashcards"]:
                            save_word(u, {**w, "next_review":str(today_dt), "origin":"Generator", "category":f"{lvl}-{w.get('category','Podstawowe')}"})
        ...
        if search.lower() in str(c.get('de','')).lower() or search.lower() in str(c.get('pl','')).lower():
            with st.expander(f"📝 {c['de']} - {c['pl']}"):
                if st.button("Usuń", key=f"del_{c['id']}", use_container_width=True): 
                    delete_word(c['id']); st.success("Usunięto!"); time.sleep(0.5); st.rerun()
...
# --- STATYSTYKI ---
    ...
    if not df.empty:
        ...
        for l in ["A1", "A2", "B1", "B2", "C1"]:
            ...
            total = len(l_df)
            know = len(l_df[l_df['next_review'].apply(lambda x: (date.fromisoformat(x)-date.today()).days >= 7 if x else False)]) if total > 0 else 0
            stats.append({"Poziom":l, "Słówek":total, "Opanowane":know, "%": f"{round((know/total)*100) if total > 0 else 0}%"})
...
# --- ADMIN ---
    ...
    st.header("👑 Panel Admina"); st.link_button("💸 Otwórz Panel Kosztów OpenAI", "https://platform.openai.com/usage", use_container_width=True)
...
# --- KONTO ---
    ...
    for i, lvl in enumerate(lvls):
        if col_d[i].button(f"Usuń {lvl}", disabled=not conf, use_container_width=True):
            ...
            st.success(f"Usunięto poziom {lvl}!"); time.sleep(1); st.rerun()

    if st.button("RESET CAŁEJ MOJEJ BAZY", type="primary", disabled=not conf, use_container_width=True):
        get_db().table("flashcards").delete().eq("username", u).execute()
        st.success("Wyczyszczono bazę!"); time.sleep(1); st.rerun()
