...
# --- PARSER AI (ODPORNY NA BŁĘDY) ---
def parse_ai_json(text):
    try:
        # Próba wyciągnięcia samej listy JSON z odpowiedzi AI
...
if st.session_state.last_choice_internal := choice: # Prosty mechanizm czyszczenia
    if st.session_state.l_c != choice:
        for k in ["n_c", "n_m", "q_c", "q_s", "f_idx", "f_flipped", "u_a", "pending"]:
    ...
    if not cards: st.success("Wszystko zrobione! 🎉")
        ...
        if st.session_state.n_m == "ask":
            with st.form("ans_form"):
                u_a_input = st.text_input("Odpowiedź:")
...
# --- MODUŁ: SKANER AI (NAPRAWIONY SILNIK PRO) ---
    ...
    if (src or up) and st.button("🚀 ANALIZUJ", use_container_width=True):
        try:
            genai.configure(api_key=API_KEY); m = genai.GenerativeModel('gemini-1.5-pro') # MODEL PRO
            ...
            else: st.error("Błąd: AI zwróciło nieprawidłowy format. Spróbuj ponownie.")
        except: st.error("Błąd połączenia z modelem Pro. Sprawdź klucz API.")
...
# --- MODUŁ: GENERATOR SŁÓW (SILNIK PRO) ---
    ...
    for i, lvl in enumerate(lvls):
        if cols[i].button(lvl, use_container_width=True):
            with st.spinner("AI Pro generuje słówka..."):
                try:
                    genai.configure(api_key=API_KEY); m = genai.GenerativeModel('gemini-1.5-pro') # MODEL PRO
                    exist = [x['de'] for x in st.session_state.flashcards[:300]]
                    p = f"Generate 25 unique German words for level {lvl}. Categories/Examples in Polish. Pomiń: {exist}. JSON: [{{'de':'...', 'pl':'...', 'category':'...', 'examples':[{{'de':'...', 'pl':'...'}}]}}]"
