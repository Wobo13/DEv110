...
# --- PARSER AI (NAPRAWA BŁĘDU AI) ---
def parse_ai_json(text):
    try:
        # Usuwa bloki markdown ```json ... ``` jeśli AI je dodało
        clean = re.sub(r'```json|```', '', text).strip()
        return json.loads(clean)
    except:
        # Próba znalezienia czegokolwiek co wygląda jak lista [ ... ]
...
# --- MODUŁ: POWTÓRKI / TRENING (ZAMROŻONE) ---
if choice in ["📅 Powtórki", "🚀 Trening"]:
    ...
    if not cards: st.success("Czysto! 🎉")
        ...
        if st.session_state.n_m == "ask":
            with st.form("ans_f"):
                u_a_input = st.text_input("Twoja odpowiedź:"); ok = st.form_submit_button("Sprawdź", use_container_width=True)
...
# --- MODUŁ: FISZKI (ZAMROŻONE) ---
elif choice == "🎴 Fiszki":
    update_activity("Fiszki"); kats = ["Wszystkie"] + sorted(list(set([c.get("category","Inne") for c in st.session_state.flashcards])))
    sel_kat = st.selectbox("🎯 Kategoria:", kats); cards = [c for c in st.session_state.flashcards if sel_kat == "Wszystkie" or c.get("category") == sel_kat]
    if cards:
        ...
        if st.session_state.f_flipped and c.get("examples"):
            for ex in c["examples"]: ex_html += f"<div style='margin-top:15px; border-top:1px solid #444; padding-top:10px;'><span style='color:#FFEB3B; font-weight:bold;'>🇩🇪 {ex['de']}</span><br><span style='color:#FFFFFF; font-style:italic;'>🇵🇱 {ex['pl']}</span></div>"
...
# --- MODUŁ: SKANER AI (NAPRAWIONY) ---
    ...
    if (src or up) and st.button("🚀 ANALIZUJ", use_container_width=True):
        try:
            ...
            res = m.generate_content(["Extract vocabulary. Return ONLY JSON list: [{'de':'...', 'pl':'...', 'category':'Skaner', 'examples':[{'de':'...', 'pl':'...'}]}]", Image.open(src or up).convert("RGB")])
            data = parse_ai_json(res.text)
            if data:
                st.session_state.pending = data; st.session_state.user_data["historical_cost"] += 0.005; st.rerun()
            else: st.error("Błąd AI: Nieczytelny format danych.")
        except: st.error("Błąd połączenia AI.")
...
# --- MODUŁ: GENERATOR SŁÓW (NAPRAWIONY) ---
        ...
        if cols[i].button(lvl, use_container_width=True):
            with st.spinner("AI generuje..."):
                try:
                    ...
                    Category MUST be in Polish. Examples in Polish. Pomiń: {exist}. 
...
# --- MODUŁ: ADMIN (ZAMROŻONE METRYKI) ---
    ...
    st.divider(); st.subheader("📊 Popularność Modułów"); total_g = sum(global_time.values())
    if total_g > 0:
    ...
    with st.expander("🔑 Zmień hasło"):
        with st.form("p_c"):
            ...
            if st.form_submit_button("Zmień"):
                db = load_j(AUTH_FILE, {})
                if db[u] == hash_pw(o) and n1 == n2: db[u]=hash_pw(n1); save_j(AUTH_FILE, db); st.success("Zmieniono!")
