...
# --- 7. START (V2.7 - Advanced Stats & Word Spotlight) ---
...
if current_choice == "🏠 Start":
    ...
    # Dane do wykresu poziomów (0-5)
    levels = [c.get('level', 0) for c in all_c]
    level_counts = {i: levels.count(i) for i in range(6)}
    df_levels = pd.DataFrame({
        "Poziom": [f"Lvl {i}" for i in range(6)],
        "Słówka": [level_counts[i] for i in range(6)]
    }).set_index("Poziom")
    ...
    col1.metric(f"Baza ({current_lang_name})", len(all_c))
    ...
    # 3. UKŁAD GŁÓWNY (ANALIZA VS ZADANIA)
    c1, c2 = st.columns([1.1, 1])
    
    with c1:
        st.markdown(f"### 📊 Rozkład Wiedzy")
        # Wyświetlamy wykres słupkowy postępu poziomów
        st.bar_chart(df_levels, height=200, use_container_width=True)
        st.caption("Poziomy: 0 (nowe) do 5 (opanowane na stałe).")

        # --- SŁÓWKO POD LUPĄ (FLASHCARD SPOTLIGHT) ---
        if due_cards:
            st.write("")
            with st.container(border=True):
                # Losujemy jedno słówko z tych, które są do powtórki (zmieniane co sesję)
                spot_word = random.choice(due_cards)
                st.markdown("🔍 **Słówko pod lupą:**")
                st.markdown(f"<h3 style='margin:0; color:#4CAF50;'>{spot_word['de']}</h3>", unsafe_allow_html=True)
                
                with st.expander("Sprawdź odpowiedź"):
                    st.write(f"🎯 **{spot_word['pl']}**")
                    if st.button("Idź do powtórek 🚀", use_container_width=True):
                        st.session_state.choice = "📅 Powtórki"
                        st.rerun()
        else:
            st.success("✨ Wszystkie słówka masz w małym palcu!")
    ...
    with c2:
        ...
        with st.container(border=True):
            ...
            else: st.markdown("✨ *Baza jest opanowana!*")
    ...
    quotes_db = {
        ...
        "cs": [{"orig": "Kolik jazyků znáš, tolikrát jsi člověkem.", "pl": "Ilu języków się nauczysz, tyle razy jesteś člověkem."}]
    }
