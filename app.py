...
# --- 📸 SKANER AI ---
    ...
    if "pending" in st.session_state:
        ...
        if st.button("✅ ZAPISZ DO BAZY", use_container_width=True):
            ...
            st.session_state.success_msg = f"🎉 Dodano {added} słówek!"
            st.rerun()
...
# --- ➕ DODAJ / 📖 SŁOWNIK ---
    ...
    cats = sorted(list(set([c.get("category", "Inne") for c in st.session_state.flashcards])))
    f_kat = st.selectbox("Filtruj:", ["Wszystkie"] + cats)
...
# --- 📊 STATYSTYKI ---
    ...
    if df.empty:
        st.warning("Baza słówek jest pusta.")
    else:
        c1, c2, c3 = st.columns(3)
        c1.metric("Łącznie słówek", len(df))
        c2.metric("Passa", f"{st.session_state.user_data['streak']} dni")
...
# --- 👑 ADMIN ---
    ...
    adm_list = []
    global_time = {m: 0.0 for m in MODULE_ORDER}
    for usr in users:
...
# --- ⚙️ MOJE KONTO ---
    ...
    with st.expander("🔑 Zmień hasło"):
        with st.form("pw_f"):
            o = st.text_input("Stare", type="password")
            n = st.text_input("Nowe", type="password")
            cp = st.text_input("Powtórz", type="password")
