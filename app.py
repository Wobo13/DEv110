...
st.sidebar.caption(f"🚀 Wersja: {APP_VERSION}") # PRZYWRÓCONO WERSJĘ
...
if choice in ["📅 Powtórki", "🚀 Trening"]:
    ...
    if cards:
        ...
        exs = f"<div style='color:#00ff00;font-size:0.85em;'>{format_ex(c.get('examples',[])).replace('\n','<br>')}</div>" if st.session_state.f_flipped else ""
...
# --- MODUŁ: SKANER AI (PRZYWRÓCONE WGRYWANIE PLIKÓW) ---
elif choice == "📸 Skaner AI":
    update_activity("Skaner")
    st.header("📸 Skaner AI")
    # PRZYWRÓCONO: camera + file_uploader
    src = st.camera_input("Zrób zdjęcie podręcznika")
    uploaded = st.file_uploader("Lub wybierz zdjęcie z galerii", type=['png', 'jpg', 'jpeg'])
    final_src = src or uploaded
    
    if final_src and st.button("🚀 ANALIZUJ", use_container_width=True):
        try:
            genai.configure(api_key=API_KEY); m = genai.GenerativeModel('gemini-2.5-flash')
            res = m.generate_content(["Zwróć JSON: [{'de':'...', 'pl':'...', 'category':'PL', 'examples':[{'de':'...', 'pl':'...'}]}]", Image.open(final_src).convert("RGB")])
        ...
        except: st.error("Błąd AI. Sprawdź połączenie.")
        ...
        if cols[i].button(lvl, use_container_width=True):
            with st.spinner("Generowanie..."):
                try:
                    ...
                    p = f"""Generate 25 German words {lvl}. Categories/Examples in Polish. 
                    Pomiń: {exist}. Return ONLY JSON: [{{'de':'...', 'pl':'...', 'category':'...', 'examples':[{{'de':'...', 'pl':'...'}}]}}]"""
    ...
    with st.form("a"):
        de, pl, kat = st.text_input("DE"), st.text_input("PL"), st.text_input("Kategoria")
    ...
    if not df.empty:
        ...
        df['status'] = df['next_review'].apply(check_st); c3.metric("Opanowane", len(df[df['status']=="Opanowane"]))
