...
# --- KONFIGURACJA ---
APP_VERSION = "V181 (Review Freeze & Generator Examples)"
ADMIN_USER = "wobo"
...
def normalize_text(t):
    """Normalizuje tekst do sprawdzania (umlauty, ss, interpunkcja)."""
    if not t: return ""
    t = str(t).lower().strip()
...
def check_test_answer(u_ans, q_obj):
    """Sprawdza odpowiedź w module Testy (DE lub PL)."""
    user = normalize_text(u_ans)
...
if choice in ["📅 Powtórki", "🚀 Trening"]:
    ...
    # Inicjalizacja listy do powtórek (zabezpieczenie przed przeskakiwaniem)
    ...
    if not cards_to_show:
        ...
        idx_pointer = st.session_state.n_idx
        if idx_pointer >= len(cards_to_show):
            st.success("Koniec sesji! Odśwież moduł, aby zacząć od nowa.")
        else:
            orig_idx, c = cards_to_show[idx_pointer]
            
            st.write(f"Słówek w kolejce: **{len(cards_to_show) - idx_pointer}**")
            ...
            if st.session_state.n_m == "ask":
                with st.form("ans_f"):
                    ...
                    if st.form_submit_button("Sprawdź", use_container_width=True):
                        st.session_state.u_a, st.session_state.n_m = u_in, "res"
                        st.rerun()
            else:
        ...
        if "test_q" not in st.session_state:
                    ...
                    if len(filtered) < 5:
                        ...
                        try:
                            ...
                            if data and "questions" in data:
                                st.session_state.test_q = data["questions"][:len(sample)]
            ...
            if t_idx < len(qs):
                ...
                if q['type'] == "LUKA":
                    ...
                    u_ans = st.radio("Wybierz opcję:", opts, key=f"t_{t_idx}")
                ...
                if st.button("Powrót", use_container_width=True):
                    del st.session_state.test_q; st.rerun()
...
# --- 📸 SKANER AI ---
    ...
    if "pending" in st.session_state:
        ...
        if st.button("✅ ZAPISZ", use_container_width=True):
            ...
            force_save(); del st.session_state.pending; st.success(f"Dodano {added}!"); time.sleep(1); st.rerun()

# --- 📦 GENERATOR SŁÓW (Naprawiony 25 słów + Przykłady) ---
    ...
    for i, lvl in enumerate(lvls):
        if cols[i].button(lvl, key=lvl, use_container_width=True):
            with st.spinner(f"Generowanie 25 słówek {lvl}..."):
                try:
                    my_w = [x['de'].lower() for x in st.session_state.flashcards]
                    # Wzmocniony prompt wymuszający liczbę i przykłady
                    prompt = f"""Generate EXACTLY 25 unique German words for level {lvl} that are NOT in this list: {my_w[:100]}.
                    For EACH word provide: 
                    1. 'de': the word, 
                    2. 'pl': translation, 
                    3. 'category': Polish category name,
                    4. 'examples': A list containing EXACTLY ONE object with 'de' and 'pl' sentence using that word.
                    Output as JSON object with key 'flashcards'."""
                    ...
                    if isinstance(data, dict) and "flashcards" in data:
                        ...
                        st.success(f"Dodano {added} nowych słówek ze zdaniami!")
...
# --- ⚙️ MOJE KONTO ---
    ...
    for i, lvl in enumerate(lvls):
        if col_d[i].button(f"Usuń {lvl}", disabled=not conf, use_container_width=True):
            ...
            force_save(); st.success(f"Usunięto {before - len(st.session_state.flashcards)} słówek!"); st.rerun()
